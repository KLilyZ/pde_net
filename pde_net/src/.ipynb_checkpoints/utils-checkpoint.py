...
#     return coes
def get_label_coe(max_order, resolution):
    """labels of PDE coefficients, ordered as (0,0), (0,1), (1,0), (0,2), (1,1), (2,0)"""
    x_cord = np.linspace(0, 2 * math.pi, num=resolution)
    y_cord = np.linspace(0, 2 * math.pi, num=resolution)
    mesh_x, mesh_y = np.meshgrid(x_cord, y_cord)

    coe_dict = dict()
    # Keys are defined as "{x_order}{y_order}"
    coe_dict['10'] = 0.5 * (np.cos(mesh_y) + mesh_x * (2 * math.pi - mesh_x) * np.sin(mesh_x)) + 0.6
    coe_dict['01'] = 2 * (np.cos(mesh_y) + np.sin(mesh_x)) + 0.8
    coe_dict['20'] = 0.2 * np.ones((resolution, resolution))
    coe_dict['02'] = 0.3 * np.ones((resolution, resolution))
    
    # Placeholder for terms not explicitly defined (like 00 for u, 11 for u_xy)
    others = np.zeros((resolution, resolution))

    coes = []

    # Define the desired output order of (x_order, y_order) tuples
    # This list explicitly specifies the order you requested: (0,0), (0,1), (1,0), (0,2), (1,1), (2,0)
    desired_order_tuples = []
    
    # Handle order 0 (u term)
    if max_order >= 0:
        desired_order_tuples.append((0, 0)) # Represents u (0th order derivative)

    # Handle order 1 (u_x, u_y terms) - Order as (0,1), (1,0)
    if max_order >= 1:
        desired_order_tuples.append((0, 1)) # Represents u_y
        desired_order_tuples.append((1, 0)) # Represents u_x

    # Handle order 2 (u_xx, u_xy, u_yy terms) - Order as (0,2), (1,1), (2,0)
    if max_order >= 2:
        desired_order_tuples.append((0, 2)) # Represents u_yy
        desired_order_tuples.append((1, 1)) # Represents u_xy
        desired_order_tuples.append((2, 0)) # Represents u_xx

    # You can extend this for higher max_order if needed, following the same pattern.
    # For instance, if max_order >= 3: (0,3), (1,2), (2,1), (3,0)
    # If max_order >= 4: (0,4), (1,3), (2,2), (3,1), (4,0)

    for ord_i, ord_j in desired_order_tuples:
        key = "{:.0f}{:.0f}".format(ord_i, ord_j)
        if key in coe_dict:
            coes.append(coe_dict.get(key))
        else:
            # If the specific (x_order, y_order) is not in coe_dict, append 'others'
            coes.append(others)
