...
def get_param_dic_frozen(summary_dir, current_step, epochs):
    file_dir = "ckpt/step_{}/pdenet-frozen-{}.ckpt".format(current_step, epochs)
    checkpoint_dir = os.path.join(summary_dir, file_dir)
    param_dict = load_checkpoint(checkpoint_dir)
    return param_dict
...
def plot_coe_label(coes, img_dir, prefix="coe", step=0, title="coes"):
    """plot coefficients of PDE"""
    make_dir(img_dir)
    plt.rcParams['figure.figsize'] = (12, 3)
    # if all coffient need plot, then num_coe, _, _ = np.shape(coes)
    ...
    for idx, coe in enumerate(coes_2d):
        ...
        if idx < 2:
            img = ax.imshow(coe, vmin=coe.min(), vmax=coe.max(), cmap=plt.get_cmap("turbo"), origin='lower')
        else:
            img = ax.imshow(coe, vmin=-0.75, vmax=0.75, cmap=plt.get_cmap("turbo"), origin='lower')
        plt.axis('off')
...
def plot_test_error_0(problem, loss_fn, item, step, mesh_size, figure_out_dir="./figure"):
    """Plot the test error"""
    ...
    plt.subplot(1, 3, 1)
    plt.title("label")
    plt.scatter(x_grid, y_grid, c=plt_y, cmap=plt.get_cmap("turbo"), vmin=min(plt_y[:]), vmax=max(plt_y[:]))
    plt.colorbar()

    plt_y_predict = y_predict.asnumpy()[0, 0, :, :].reshape(-1, 1)
    plt.subplot(1, 3, 2)
    plt.title("prediction")
    plt.scatter(x_grid, y_grid, c=plt_y_predict, cmap=plt.get_cmap("turbo"), vmin=min(plt_y_predict[:]),
    ...
    plt.subplot(1, 3, 3)
    plt.title("error")
    plt.scatter(x_grid, y_grid, c=error_val, cmap=plt.get_cmap("turbo"), vmin=min(error_val[:]), vmax=max(error_val[:]))
    plt.colorbar()
...
# def plot_extrapolation_error(config, dataset,color, max_step=40):
...
#     plt.semilogy(plot_data[:, 0], plot_data[:, 1], color=color)
#     plt.semilogy(plot_data[:, 0], plot_data[:, 2], color=color)
#     plt.fill_between(plot_data[:, 0], plot_data[:, 1], plot_data[:, 2], facecolor=color, alpha=0.5)
#     plt.xlim(1, max_step)
#     plt.ylim(0.01, 100)
#     plt.savefig(os.path.join(config["figure_out_dir"], 'extrapolation.jpg'))

def _extapolation(config, param_dict,if_fronzen,extra_step, test_data_iterator):
    """long time prediction test for given ckpt"""
    model = init_model(config)
    if extra_step == 1 or if_fronzen:
        model.if_fronzen = True
...
def plot_extrapolation_error(config, param_dict_active,param_dict_frozen,dataset, max_step=40):
    """ plot extrapolation error """
    error_active_data = []
    error_frozen_data = []
    plot_active_data = np.zeros([max_step, 3])
    plot_frozen_data = np.zeros([max_step, 3])
    for i in range(1, max_step + 1):
        test_dataset = dataset.create_test_dataset(i)
        test_data_iterator = test_dataset.create_dict_iterator()
        print(test_data_iterator)
        error_active = _extapolation(config, param_dict_active,False, i, test_data_iterator)
        error_active_data.append(error_active)
        p25 = np.percentile(error_active, 25)
        p75 = np.percentile(error_active, 75)
        print("step = {:.0f}, p25 = {:.5f}, p75 = {:.5f}".format(i, p25, p75))
        plot_active_data[i - 1, :] = [i, p25, p75]
        
        # 第二次使用迭代器，需要重新创建
        test_data_iterator_frozen = test_dataset.create_dict_iterator() # 重新创建迭代器
        print(test_data_iterator_frozen)
        error_frozen = _extapolation(config, param_dict_frozen, True, i, test_data_iterator_frozen)
        error_frozen_data.append(error_frozen)
        p25_frozen = np.percentile(error_frozen, 25) # 变量名可以区分开
        p75_frozen = np.percentile(error_frozen, 75) # 变量名可以区分开
        print("step = {:.0f}, p25 = {:.5f}, p75 = {:.5f}".format(i, p25_frozen, p75_frozen))
        plot_frozen_data[i - 1, :] = [i, p25_frozen, p75_frozen]

    plt.semilogy(plot_active_data[:, 0], plot_active_data[:, 1], color='orange')
    plt.semilogy(plot_active_data[:, 0], plot_active_data[:, 2], color='orange')
    plt.fill_between(plot_active_data[:, 0], plot_active_data[:, 1], plot_active_data[:, 2], facecolor='orange', alpha=0.5)
    
    plt.semilogy(plot_frozen_data[:, 0], plot_frozen_data[:, 1], color='lightblue')
    plt.semilogy(plot_frozen_data[:, 0], plot_frozen_data[:, 2], color='lightblue')
    plt.fill_between(plot_frozen_data[:, 0], plot_frozen_data[:, 1], plot_frozen_data[:, 2], facecolor='lightblue', alpha=0.5)
